...
class PedestrianDataset(VisionDataset):
    def __init__(self, base, is_train=True, resolution=(160, 4, 250), bounds=(-500, 500, -320, 320, 0, 2), final_dim: tuple = (720, 1280), resize_lim: list = (0.8, 1.2)):
        ...
        self.vox_util = vox.VoxelUtil(self.Y, self.Z, self.X, scene_centroid=self.scene_centroid, bounds=self.bounds, assert_cube=False)
        if self.is_train:
    ...
    def prepare_gt(self):
        ...
        for fname in sorted(os.listdir(os.path.join(self.root, 'annotations_positions'))):
            ...
            for single_pedestrian in all_pedestrians:
                def is_in_cam(cam):
                    return not (single_pedestrian['views'][cam]['xmin'] == -1 and single_pedestrian['views'][cam]['xmax'] == -1 and
                                single_pedestrian['views'][cam]['ymin'] == -1 and single_pedestrian['views'][cam]['ymax'] == -1)
                ...
                for pedestrian in all_pedestrians:
                    ...
                    for cam in range(self.num_cam):
                        if itemgetter('xmin', 'ymin', 'xmax', 'ymax')(pedestrian['views'][cam]) != (-1, -1, -1, -1):
                            ...
                            num_imgs_bbox += 1
                self.world_gt[frame] = (torch.tensor(world_pts, dtype=torch.float32), torch.tensor(world_pids, dtype=torch.float32))
                self.imgs_gt[frame] = {}
                for cam in range(self.num_cam):
    ...
    def get_image_data(self, frame, cameras):
        ...
        return torch.stack(imgs), torch.stack(intrins), torch.stack(extrins), torch.stack(centers), torch.stack(offsets), torch.stack(sizes), torch.stack(pids), torch.stack(valids)
    ...
    def __getitem__(self, index):
        ...
        imgs, intrins, extrins, centers_img, offsets_img, sizes_img, pids_img, valids_img = self.get_image_data(frame, cameras)
        ...
        item = {'img': imgs, 'intrinsic': intrins, 'extrinsic': extrins, 'ref_T_global': worldgrid_T_worldcoord, 'frame': frame // self.base.frame_step, 'sequence_num': int(0), 'grid_gt': grid_gt}
        target = {'valid_bev': valid_bev, 'center_bev': center_bev, 'offset_bev': offset_bev, 'pid_bev': pid_bev, 'center_img': centers_img, 'offset_img': offsets_img,
                  'size_img': sizes_img, 'valid_img': valids_img, 'pid_img': pids_img}
        return item, target