...
def prepare_gt(root):
    ...
    vox_util = vox.VoxelUtil(Y, Z, X, scene_centroid=torch.tensor(scene_centroid, device='cuda:0').reshape([1, 3]), bounds=bounds, assert_cube=False)
    ...
    for town_path in sorted(town_path_all):
        ...
        for _, camera_name in enumerate(camera_names):
            ...
            intrinsic = torch.tensor(calibration_dict['intrinsic_matrix'])
            intrinsic = geom.merge_intrinsics(*geom.split_intrinsics(intrinsic.unsqueeze(0))).squeeze()
            extrinsic = torch.tensor(calibration_dict['extrinsic_matrix'])
            t = extrinsic[:3, 3:]
            r = extrinsic[:3, :3]
            t = torch.tensor([t[1], -t[2], t[0]])
            change = torch.tensor([[0, 1, 0], [0, 0, -1], [1, 0, 0]])
            r = torch.matmul(change.float(), r)
            cameras[camera_name] = {'rot': r, 'tran': t, 'intrin': intrinsic}
        rots = torch.stack([c['rot'] for c in cameras.values()])
        trans = torch.stack([c['tran'] for c in cameras.values()])
        camXs_T_global = geom.merge_rt(rots, trans.squeeze(-1))
        global_T_camXs = torch.inverse(camXs_T_global)
