...
class Segnet(nn.Module):
    def __init__(self, Y, Z, X, do_rgbcompress=True, rand_flip=False, latent_dim=256, feat2d_dim=128, num_classes=None, num_cameras=None, z_sign=1, encoder_type='swin_t', device=torch.device('cuda')):
    ...
    def forward(self, rgb_cams, pix_T_cams, cams_T_global, vox_util, ref_T_global, prev_bev=None):
        ...
        feat_mems_ = vox_util.unproject_image_to_mem(feat_cams_, utils.basic.matmul2(featpix_T_cams_, cams_T_ref_), cams_T_ref_, Y, Z, X, xyz_refA=None, z_sign=self.z_sign)
