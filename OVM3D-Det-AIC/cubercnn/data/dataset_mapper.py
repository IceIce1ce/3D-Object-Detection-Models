...
class DatasetMapper3D(DatasetMapper):
    def __call__(self, dataset_dict):
        ...
        if "annotations" in dataset_dict:
            ...
            annos = [transform_instance_annotations(obj, transforms, K=K) for obj in dataset_dict.pop("annotations") if obj.get("iscrowd", 0) == 0]
            instances = annotations_to_instances(annos, image_shape, unknown_categories)
            dataset_dict["instances"] = detection_utils.filter_empty_instances(instances)
        return dataset_dict

_M1 = np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]])
_M2 = np.array([[-1.,  0.,  0.], [ 0., -1.,  0.], [ 0.,  0.,  1.]])
...
def transform_instance_annotations(annotation, transforms, *, K):
    ...
    if annotation['center_cam'][2] != 0:
        ...
        annotation["center_cam_proj"][0:2] = transforms.apply_coords(point2D[np.newaxis][:, :2])[0].tolist()
