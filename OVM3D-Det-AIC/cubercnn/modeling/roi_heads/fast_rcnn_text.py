...
class FastRCNNOutputs_text(FastRCNNOutputLayers):

    def __init__(self, cfg, input_shape):
        super().__init__(cfg, input_shape)
        input_size = input_shape.channels * (input_shape.width or 1) * (input_shape.height or 1)
        self.cls_score = nn.Linear(input_size, 768)

    def forward(self, x, text_embeddings):
        if x.dim() > 2:
            x = torch.flatten(x, start_dim=1)
        embedding = self.cls_score(x)
        scores = torch.einsum('nc,kc->nk', embedding, text_embeddings)
