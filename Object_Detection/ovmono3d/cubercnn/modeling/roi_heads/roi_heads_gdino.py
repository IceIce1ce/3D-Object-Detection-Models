...
class ROIHeads3DGDINO(ROIHeads3D):
    @configurable
    def __init__(self, *, ignore_thresh: float, cube_head: nn.Module, cube_pooler: nn.Module, loss_w_3d: float, loss_w_xy: float, loss_w_z: float, loss_w_dims: float,
                 loss_w_pose: float, loss_w_joint: float, use_confidence: float, inverse_z_weight: bool, z_type: str, pose_type: str, cluster_bins: int, priors = None,
                 dims_priors_enabled = None, dims_priors_func = None, disentangled_loss=None, virtual_depth=None, virtual_focal=None, test_scale=None, allocentric_pose=None,
                 chamfer_pose=None, scale_roi_boxes=None, **kwargs):
        super().__init__(ignore_thresh=ignore_thresh, cube_head=cube_head, cube_pooler=cube_pooler, loss_w_3d=loss_w_3d, loss_w_xy=loss_w_xy, loss_w_z=loss_w_z, loss_w_dims=loss_w_dims,
                         loss_w_pose=loss_w_pose, loss_w_joint=loss_w_joint, use_confidence=use_confidence, inverse_z_weight=inverse_z_weight, z_type=z_type, pose_type=pose_type,
                         cluster_bins=cluster_bins, priors=priors, dims_priors_enabled=dims_priors_enabled, dims_priors_func=dims_priors_func, disentangled_loss=disentangled_loss,
                         virtual_depth=virtual_depth, virtual_focal=virtual_focal, test_scale=test_scale, allocentric_pose=allocentric_pose, chamfer_pose=chamfer_pose,
                         scale_roi_boxes=scale_roi_boxes, **kwargs)
        self.groundingdino_model = load_model("./configs/GroundingDINO_SwinB_cfg.py",  "./checkpoints/groundingdino_swinb_cogcoor.pth", cpu_only=False)
    ...
    def forward(self, images, features, proposals, Ks, im_scales_ratio, targets=None, category_list=None):
        ...
        if self.training:
            ...
            if not filtered_texts:
                ...
                target.pred_classes = torch.tensor([], dtype=torch.int64)
                target.pred_boxes = Boxes(torch.tensor([], dtype=torch.float32).view(-1, 4))
                target.scores = torch.tensor([], dtype=torch.float32)
                target = target.to(device=pred_instances[0].scores.device)
            else:
                configs = {"groundingdino_model": self.groundingdino_model, "image": images[0][[2, 1, 0], :, :], "text_prompt": filtered_texts, "box_threshold": 0.001,
                           "text_threshold": 0.25, "token_spans": None, "cpu_only": False}
...
def grounding_dino_inference_detector(config):
    ...
    boxes_filt, pred_phrases, all_logits = get_grounding_output(groundingdino_model, image, text_prompt, box_threshold, text_threshold, cpu_only, token_spans=eval(f"{token_spans}"))
    h, w = image.shape[1:]
...
def get_phrase_logits_from_token_logits(token_logits: torch.Tensor, tokenized: Dict, tokenizer: AutoTokenizer, cap_list: List):
