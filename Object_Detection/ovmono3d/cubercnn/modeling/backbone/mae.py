...
class MAEBackbone(Backbone):
    ...
    def resize_pos_embed(self, image_size):
        ...
        pos_embed = get_2d_sincos_pos_embed(embed_dim, (self.feat_h, self.feat_w), add_cls_token=True)
        device = self.vit.embeddings.patch_embeddings.projection.weight.device
        self.vit.embeddings.position_embeddings = nn.Parameter(torch.from_numpy(pos_embed).float().unsqueeze(0).to(device=device), requires_grad=False)
    ...
    def forward(self, images):
        ...
        encoder_outputs = self.vit.encoder(embedding_output, head_mask=head_mask, output_attentions=self.vit.config.output_attentions, output_hidden_states=True, return_dict=self.vit.config.return_dict)
        outputs = {}
        for idx, layer_i in enumerate(self.multilayers):
            x_i = encoder_outputs.hidden_states[layer_i]
            x_i = tokens_to_output(self.output, x_i[:, 1:], x_i[:, 0], (self.feat_h, self.feat_w))
...
def build_mae_backbone(cfg, input_shape: ShapeSpec, priors=None):
    ...
    bottom_up = MAEBackbone(cfg, input_shape, checkpoint=checkpoint, output=output, layer=layer, return_multilayer=return_multilayer)
