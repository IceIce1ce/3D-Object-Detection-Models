...
from detectron2.data.build import filter_images_with_only_crowd_annotations, build_batch_data_loader, trivial_batch_collator
import random
...
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None, dataset_id_to_src=None):
    if dataset is None:
        dataset = get_detection_dataset_dicts(cfg.DATASETS.TRAIN, filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS, min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
                                              if cfg.MODEL.KEYPOINT_ON else 0, proposal_files=cfg.DATASETS.PROPOSAL_FILES_TRAIN if cfg.MODEL.LOAD_PROPOSALS else None)
    ...
    if sampler is None:
        ...
        if sampler_name == "TrainingSampler" and not balance_datasets:
            ...
            repeat_factors = repeat_factors_from_category_frequency(dataset, cfg.DATALOADER.REPEAT_THRESHOLD)
    ...
    return {"dataset": dataset, "sampler": sampler, "mapper": mapper, "total_batch_size": cfg.SOLVER.IMS_PER_BATCH, "aspect_ratio_grouping": cfg.DATALOADER.ASPECT_RATIO_GROUPING, "num_workers": cfg.DATALOADER.NUM_WORKERS}
...
def build_detection_train_loader(dataset, *, mapper, sampler=None, total_batch_size, aspect_ratio_grouping=True, num_workers=0):
    ...
    return build_batch_data_loader(dataset, sampler, total_batch_size, aspect_ratio_grouping=aspect_ratio_grouping, num_workers=num_workers)
...
def _test_loader_from_config(cfg, dataset_name, mode, mapper=None):
    ...
    dataset = get_detection_dataset_dicts(
        dataset_name,
        filter_empty=False,
        oracle2d=[getattr(getattr(cfg.DATASETS.ORACLE2D_FILES[cfg.DATASETS.ORACLE2D_FILES.EVAL_MODE], mode), x) for x in dataset_name] if cfg.TEST.ORACLE2D else None,
                                  proposal_files=[cfg.DATASETS.PROPOSAL_FILES_TEST[list(cfg.DATASETS.TEST).index(x)] for x in dataset_name] if cfg.MODEL.LOAD_PROPOSALS else None)
    if mapper is None:
...
def build_detection_test_loader(dataset, *, mapper, sampler=None, num_workers=0):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, num_workers=num_workers, batch_sampler=batch_sampler, collate_fn=trivial_batch_collator)
    return data_loader
