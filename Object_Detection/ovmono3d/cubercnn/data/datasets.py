...
def get_filter_settings_from_cfg(cfg=None):
    if cfg is None:
        return {'category_names': [], 'ignore_names': [], 'truncation_thres': 0.99, 'visibility_thres': 0.01, 'min_height_thres': 0.00, 'max_height_thres': 1.50,
                'modal_2D_boxes': False, 'trunc_2D_boxes': False, 'max_depth': 1e8}
    else:
        return {'category_names': cfg.DATASETS.CATEGORY_NAMES, 'ignore_names': cfg.DATASETS.IGNORE_NAMES, 'truncation_thres': cfg.DATASETS.TRUNCATION_THRES,
                'visibility_thres': cfg.DATASETS.VISIBILITY_THRES, 'min_height_thres': cfg.DATASETS.MIN_HEIGHT_THRES, 'modal_2D_boxes': cfg.DATASETS.MODAL_2D_BOXES,
                'trunc_2D_boxes': cfg.DATASETS.TRUNC_2D_BOXES, 'max_depth': cfg.DATASETS.MAX_DEPTH, 'max_height_thres': 1.50}
...
def simple_register(dataset_name, filter_settings, filter_empty=False, datasets_root_path=None):
    ...
    DatasetCatalog.register(dataset_name, lambda: load_omni3d_json(path_to_json, path_to_image_root,  dataset_name, filter_settings, filter_empty=filter_empty))
...
class Omni3D(COCO):
    def __init__(self, annotation_files, filter_settings=None):
        ...
        if filter_settings is None:
            self.dataset['categories'] = [cats_master[i] for i in np.argsort(cats_ids_master)]
        else:
            ...
            if len(filter_settings['category_names']) > 0:
                self.dataset['categories'] = [cats_master[i] for i in np.argsort(cats_ids_master) if cats_master[i]['name'] in filter_settings['category_names']]
            else:
                self.dataset['categories'] = [cats_master[i] for i in np.argsort(cats_ids_master)]
...
def register_and_store_model_metadata(datasets, output_dir, filter_settings=None):
    ...
    util.save_json(output_file, {'thing_classes': thing_classes, 'thing_dataset_id_to_contiguous_id': id_map})
...
def load_omni3d_json(json_file, image_root, dataset_name, filter_settings, filter_empty=False):
    ...
    dataset_dicts = []
    ann_keys = ["bbox", "bbox3D_cam", "bbox2D_proj", "bbox2D_trunc", "bbox2D_tight", "center_cam", "dimensions", "pose", "R_cam", "category_id"]
