...
def evaluate_predictions(dataset_names, prediction_paths, filter_settings, output_dir, category_path, eval_mode="novel", iter_label='final'):
    os.makedirs(output_dir, exist_ok=True)
    setup_logger(output=output_dir, name="cubercnn")
    setup_categories(category_path)
    thing_classes = ['monitor', 'bag', 'dresser', 'board', 'printer', 'keyboard', 'painting', 'drawers', 'microwave', 'computer', 'kitchen pan', 'potted plant', 'tissues', 'rack',
                     'tray', 'toys', 'phone', 'podium', 'cart', 'soundsystem', 'fireplace', 'tram']
    filter_settings['category_names'] = thing_classes
    eval_helper = Omni3DEvaluationHelper(dataset_names=dataset_names, filter_settings=filter_settings, output_folder=output_dir, iter_label=iter_label, only_2d=False, eval_categories=thing_classes)
...
def main():
    ...
    prediction_paths = {"SUNRGBD_test_novel": "./output/ovmono3d_geo/SUNRGBD_test_novel.pth",
    ...
    filter_settings = {'visibility_thres': 0.33333333, 'truncation_thres': 0.33333333, 'min_height_thres': 0.0625, 'max_depth': 100000000.0, 'category_names': None,
                       'ignore_names': ['dontcare', 'ignore', 'void'], 'trunc_2D_boxes': True, 'modal_2D_boxes': False, 'max_height_thres': 1.5}
    output_dir = "./output/ovmono3d_geo"
    category_path = "./configs/category_meta.json"
    evaluate_predictions(dataset_names=dataset_names, prediction_paths=prediction_paths, filter_settings=filter_settings, output_dir=output_dir, category_path=category_path, eval_mode="novel", iter_label='final')
