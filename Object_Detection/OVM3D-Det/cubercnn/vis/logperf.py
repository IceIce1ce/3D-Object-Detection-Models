...
def print_ap_category_histogram(dataset, results):
    ...
    N_COLS = 9
    data = list(itertools.chain(*[[cat, out["AP2D"], out["AP3D"]] for cat, out in results.items()]))
    data.extend([None] * (N_COLS - (len(data) % N_COLS)))
    data = itertools.zip_longest(*[data[i::N_COLS] for i in range(N_COLS)])
    table = tabulate(data, headers=["category", "AP2D", "AP3D"] * (N_COLS // 2), tablefmt="pipe", numalign="left", stralign="center")
...
def print_ap_analysis_histogram(results):
    ...
    table = tabulate(data, headers=["Dataset", "#iters", "AP2D", "AP3D", "AP3D@15", "AP3D@25", "AP3D@50", "AP3D-N", "AP3D-M", "AP3D-F"], tablefmt="grid", numalign="left", stralign="center")
