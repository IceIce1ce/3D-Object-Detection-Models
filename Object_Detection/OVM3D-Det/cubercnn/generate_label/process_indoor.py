...
def process_indoor(dataset, cat_prior, input_folder, output_folder):
    ...
    for idx in tqdm(range(len(dataset._dataset))):
        ...
        boxes3d, center_cam_list, dimension_list, R_cam_list = process_instances(mask, depth, K, info[im_id], cat_prior, has_ground, ground_equ)
        info[im_id].update({'boxes3d': boxes3d, 'center_cam': center_cam_list, 'dimensions': dimension_list, 'R_cam': R_cam_list})
    torch.save(info, os.path.join(input_folder, 'info_3d.pth'))

def estimate_bbox(in_pc, prior, ground_equ=None):
