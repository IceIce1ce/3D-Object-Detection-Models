...
class Tester(object):
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'single' or not self.train_cfg["save_all"]:
            ...
            load_checkpoint(model=self.model, optimizer=None, filename=checkpoint_path, map_location=self.device, logger=self.logger, to_cpu=self.dist_test)
            ...
            for checkpoint in checkpoints_list:
                load_checkpoint(model=self.model, optimizer=None, filename=checkpoint, map_location=self.device, logger=self.logger, to_cpu=self.dist_test)
                self.model.to(self.device)
                self.inference()
                self.evaluate()
    ...
    def inference(self):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.dataloader):
            ...
            dets = decode_detections(dets=dets, info=info, calibs=calibs, cls_mean_size=cls_mean_size, threshold=self.cfg.get('threshold', 0.2))
            results.update(dets)
            if self.rank == 0:
                progress_bar.update()
        comm.synchronize()
        if self.rank == 0:
            print("inference on {} images by {}/per image".format(len(self.dataloader), model_infer_time / len(self.dataloader) / self.batch_size))
            progress_bar.close()
    ...
    def save_results(self, results):
        ...
        for img_id in results.keys():
            if self.dataset_type == 'KITTI':
                ...
                output_path = os.path.join(output_dir, self.dataloader.dataset.get_sensor_modality(img_id), self.dataloader.dataset.get_sample_token(img_id) + '.txt')
