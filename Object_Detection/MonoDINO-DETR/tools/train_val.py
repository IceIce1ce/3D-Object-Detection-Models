...
from lib.helpers.dataloader_helper import build_dataloader, build_testloader
...
def main(args):
    ...
    if args.evaluate_only:
        test_set, test_loader, test_sampler = build_testloader(cfg['dataset'], batch_size=args.batch_size, dist=distributed, training=False)
        logger.info('###################  Evaluation Only  ##################')
        tester = Tester(cfg=cfg['tester'], model=model, dataloader=test_loader, logger=logger, train_cfg=cfg['trainer'], model_name=model_name, dist_test=distributed,
                        rank=cfg.local_rank, batch_size=args.batch_size)
        tester.test()
        return
    if distributed:
    ...
    train_set, train_loader, train_sampler = build_dataloader(cfg['dataset'], batch_size=args.batch_size, dist=distributed, training=True)
    test_set, test_loader, test_sampler = build_testloader(cfg['dataset'], batch_size=args.batch_size, dist=distributed, training=False)
    lr_scheduler, warmup_lr_scheduler = build_lr_scheduler(cfg['lr_scheduler'], optimizer, last_epoch=-1)
    trainer = Trainer(cfg=cfg['trainer'], model=model, optimizer=optimizer, train_loader=train_loader, lr_scheduler=lr_scheduler, warmup_lr_scheduler=warmup_lr_scheduler,
                      logger=logger, loss=loss, model_name=model_name, dist_train=distributed, tb_log=tb_log, rank=cfg.local_rank)
    tester = Tester(cfg=cfg['tester'], model=model, dataloader=test_loader, logger=logger, train_cfg=cfg['trainer'], model_name=model_name, dist_test=distributed,
                    rank=cfg.local_rank, batch_size=args.batch_size)
    ...
    if cfg.local_rank == 0:
        ...
        test_set, test_loader, test_sampler = build_testloader(cfg['dataset'], batch_size=args.batch_size, dist=distributed, training=False)
        tester = Tester(cfg=cfg['tester'], model=model, dataloader=test_loader, logger=logger, train_cfg=cfg['trainer'], model_name=model_name, dist_test=False,
                        rank=cfg.local_rank, batch_size=args.batch_size)
        logger.info('###################  Testing  ##################')
        logger.info('Batch Size: %d' % (args.batch_size))
...
if __name__ == '__main__':
    args = parse_config()
    launch(main, args.num_gpus, num_machines=args.num_machines, machine_rank=args.local_rank, dist_url=args.dist_url, args=(args,))