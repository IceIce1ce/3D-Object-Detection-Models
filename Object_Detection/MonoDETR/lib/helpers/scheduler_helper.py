...
class CosineWarmupLR(lr_sched._LRScheduler):
    ...
    def get_lr(self):
        return [self.init_lr + (base_lr - self.init_lr) * (1 - math.cos(math.pi * self.last_epoch / self.num_epoch)) / 2 for base_lr in self.base_lrs]
...
class LinearWarmupLR(lr_sched._LRScheduler):
    ...
    def get_lr(self):
        return [self.init_lr + (base_lr - self.init_lr) * self.last_epoch / self.num_epoch for base_lr in self.base_lrs]