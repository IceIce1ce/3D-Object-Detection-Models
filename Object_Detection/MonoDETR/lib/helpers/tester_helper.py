...
class Tester(object):
    ...
    def test(self):
        ...
        if self.cfg['mode'] == 'single' or not self.train_cfg["save_all"]:
            ...
            load_checkpoint(model=self.model, optimizer=None, filename=checkpoint_path, map_location=self.device, logger=self.logger)
            ...
            for checkpoint in checkpoints_list:
                load_checkpoint(model=self.model, optimizer=None, filename=checkpoint, map_location=self.device, logger=self.logger)
                self.model.to(self.device)
                self.inference()
                self.evaluate()
    ...
    def inference(self):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.dataloader):
            ...
            results.update(dets)
            progress_bar.update()
        print("inference on {} images by {}/per image".format(len(self.dataloader), model_infer_time / len(self.dataloader)))
        progress_bar.close()
